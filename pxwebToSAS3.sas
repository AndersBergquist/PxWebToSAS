proc ds2;
	package &instprog..pxwebToSAS3 / overwrite=yes;
		declare package sasuser.pxwebToSAS2 px();
		declare package sasuser.prg_nyttigheter nytta();
...
*			declare varchar(32) filnamn;
			declare varchar(1000) sqlQuery;
			declare integer finnsUpdate filFinns;
...
*			filnamn=scan(iUrl,-1);
			...
			if finnsUpdate=1 then do;
				...
				/* Buggfix 1.02 Ny kod*/
				if sLib^='work' then do;
					filFinns=nytta.finnsTabell(sLib, sTabell);
					if filFinns=0 then do;
						sqlQuery='CREATE TABLE ' || sLib || '.' || sTabell || ' AS SELECT * FROM ' || iLib || '.' || iTabell;
						sqlexec('CREATE TABLE ' || sLib || '.' || sTabell || ' AS SELECT * FROM ' || iLib || '.' || iTabell);
					end;
					else do;*buggfix 1.03, borttaget semikolon;
						sqlQuery='INSERT INTO ' || sLib || '.' || sTabell || ' SELECT * FROM ' || iLib || '.' || iTabell;
						sqlexec('INSERT INTO ' || sLib || '.' || sTabell || ' SELECT * FROM ' || iLib || '.' || iTabell);
					end;
				end;
				/* Buggfix 1.02 */
