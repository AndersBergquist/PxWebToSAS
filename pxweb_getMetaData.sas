...
proc ds2;
	package &prgLib..pxweb_getMetaData / overwrite=yes;
		...
		method getData(nvarchar(500) iUrl, integer maxCells, nvarchar(41) fullTabellNamn, nvarchar(32) tmpTable);
			...
			deleteExistingObs(fullTabellNamn, tmpTable);
...
*Senaste tid är där laghämtningen ska styras ifrån. Bra att redan nu hämtas bara senate data.;
		...
		method deleteExistingObs(nvarchar(41) fullTabellNamn, nvarchar(42) tmpTable);
			declare package sqlstmt s_tidvar;
			declare package sqlstmt s_numItem();
			declare integer rc;
			if g.finnsTabell(fullTabellNamn)=1 then do;
				s_tidvar = _new_ sqlstmt('select distinct code from meta_' || tmpTable || ' where "time"=''true''');
				s_tidvar.execute();
				...
				if rc=0 then do;
					do until(hi_finnsTid.next([tidData]));
						sqlexec('delete from meta_' || tmpTable || ' where "values"=''' || strip(tidData) || ''' and "time"=''true''');
					end;
				end;
			end;
			s_numItem = _new_ sqlstmt('select count(valueTexts) as numItem from meta_' || tmpTable);
			s_numItem.execute();
