					...
					do until(j.ISRIGHTBRACKET(tokenType) or rc^=0);
						if token='key' then do;
							...
							do until(j.ISRIGHTBRACKET(tokenType) or rc^=0);
								...
								i=i+1;
								if dtype='t' and lowCase(kolTexts) in ('år', 'vartannat år', 'kvartal', 'månad','�r', 'vartannat �r', 'm�nad') then do;
...
*Ersätter ovan så - blir tal; *  if notdigit(compress(token,'+-','d'))=1 then token=.;
...
*put c_index= token=;
		...
		declare double manad ar tid_dt;
			...
			if lowCase(tid_nm) in ('år', 'vartannat år', '�r', 'vartannat �r')  then do;
				manad=1;
			end;
			else if lowCase(tid_nm) = 'kvartal' then do;
				manad=substr(tid_cd,6,1)*3-2;
			end;
			else if lowCase(tid_nm) in ('månad', 'm�nad') then do;
