					...
					do until(j.ISRIGHTBRACKET(tokenType));
						if token='key' then do;
							...
							do until(j.ISRIGHTBRACKET(tokenType));
								...
								values=token;
								h_valuesdata.find([kolNamn values],[code values valuetexts]);
*put kolNamn= values= valueTexts=;
								sc=s_updateTmpTable.setvarchar(i, values);
if sc=1 then put 'key' i= sc= token=;
								i=i+1;
								sc=s_updateTmpTable.setvarchar(i, values);
if sc=1 then put 'key' i= sc= token=;
								i=i+1;
...
*									sc=s_updateTmpTable.setinteger(i, tid_dt);
if sc=1 then put 'key' i= sc= token=;
*									i=i+1;
								end;
								c_index=c_index+1;
						...
						if token='values' then do;
							...
							do until(j.ISRIGHTBRACKET(tokenType));
								...
								i=i+1;
								c_index=c_index+1;
if sc=1 then put 'values' i= token=;
								j.getNextToken(rc, token, tokenType, parseFlags);
							end;
						end;
						j.getNextToken(rc, token, tokenType, parseFlags);
...
put 'stmt.execute:' sc=;
		...
		declare integer tid_dt;
		declare double manad ar;
