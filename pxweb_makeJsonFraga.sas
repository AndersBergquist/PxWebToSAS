...
if values='2020M03' then put 'Delmängda av variabelerna, rundaNr=cellerPerValue:' values;
...
if values='2020M03' then put 'Delmängda av variabelerna, rundaNr=1:' values;
								stubFraga=stubFraga || '"' || values || '"';
							end;
							else do;
if values='2020M03' then put 'Delmängda av variabelerna, else do:' values;
								stubFraga=stubFraga || ', "' || values || '"';
							end;
						end;
					getMetaData.metaDataNext(title, code, text, values, valueTexts, elimination, "time");
if values='2020M04' then put 'Delmängda av variabelerna, Hämtar frånmetadata:' values iMetaData sizeMetaData;
					iMetaData=iMetaData+1;
					end;
