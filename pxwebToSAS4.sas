...
- output:
	1. L�nmnar returkod till 0 om uppdatering genomförts och 1 om den inte genomförts.
***********************************/
proc ds2;
...
******** getData varianter för att göra det så flexibelt som möjligt att hämta data. start;
		...
		method getDataStart(nvarchar(500) iUrl, nvarchar(8) SASLib, nvarchar(32) SASTabell, integer maxCells, nvarchar(32) tmpTable) returns integer;
			...
			if runtime < 60 then do;
				put antalCeller nlnum24.-l ' celler h�mtades p�' runTime 'sekunder vilket motsvarar ' cellerSek nlnum27.2-l ' celler per sekund. Returkod:' ud;
			end;
			else do;
				min=int(runTime/60);
				sek=mod(runTime,60);
				put AntalCeller nlnum24.-l ' celler h�mtades p�' min 'minuter och ' sek 'sekunder vilket motsvarar ' cellerSek nlnum27.2-l ' celler per sekund. Returkod:' ud;
			end;
			put vstring;
			return ud;
		end;
